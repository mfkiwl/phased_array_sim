...
output_file_name = 'visuals\\ga_elo_bpso_compare_msb.png'
...
#stuck_bits = average_df["Unnamed: 0"].to_numpy()
#stuck_bits = stuck_bits / total_bits  # Normalise stuck bits to be between 0 and 1
tail_nse_broken_list = average_df['avg_nse_broken'].to_numpy()
tail_nse_elo_list = average_df['avg_nse_elo'].to_numpy()
tail_nse_ga_list = average_df['avg_nse_ga'].to_numpy()
tail_nse_bpso_list = average_df['avg_nse_bpso'].to_numpy()
tail_pssl_broken_list = average_df['avg_pssl_broken'].to_numpy()
tail_pssl_elo_list = average_df['avg_pssl_elo'].to_numpy()
tail_pssl_ga_list = average_df['avg_pssl_ga'].to_numpy()
tail_pssl_bpso_list = average_df['avg_pssl_bpso'].to_numpy()
tail_pbp_broken_list = average_df['avg_pbp_broken'].to_numpy()
tail_pbp_elo_list = average_df['avg_pbp_elo'].to_numpy()
tail_pbp_ga_list = average_df['avg_pbp_ga'].to_numpy()
tail_pbp_bpso_list = average_df['avg_pbp_bpso'].to_numpy()
tail_isll_broken_list = average_df['avg_isll_broken'].to_numpy()
tail_isll_elo_list = average_df['avg_isll_elo'].to_numpy()
tail_isll_ga_list = average_df['avg_isll_ga'].to_numpy()
tail_isll_bpso_list = average_df['avg_isll_bpso'].to_numpy()

avg_nse_broken_list[-2:] = tail_nse_broken_list
avg_nse_elo_list[-2:] = tail_nse_elo_list
avg_nse_ga_list[-2:] = tail_nse_ga_list
avg_nse_bpso_list[-2:] = tail_nse_bpso_list
avg_pssl_broken_list[-2:] = tail_pssl_broken_list
avg_pssl_elo_list[-2:] = tail_pssl_elo_list
avg_pssl_ga_list[-2:] = tail_pssl_ga_list
avg_pssl_bpso_list[-2:] = tail_pssl_bpso_list
avg_pbp_broken_list[-2:] = tail_pbp_broken_list
avg_pbp_elo_list[-2:] = tail_pbp_elo_list
avg_pbp_ga_list[-2:] = tail_pbp_ga_list
avg_pbp_bpso_list[-2:] = tail_pbp_bpso_list
avg_isll_broken_list[-2:] = tail_isll_broken_list
avg_isll_elo_list[-2:] = tail_isll_elo_list
avg_isll_ga_list[-2:] = tail_isll_ga_list
avg_isll_bpso_list[-2:] = tail_isll_bpso_list



# 3 subplots, one for NSe, one for PSSL and one for PBP
fig, axs = plt.subplots(3, 1, figsize=(6, 9), gridspec_kw={'height_ratios': [1, 1, 1]})
...
axs[2].plot(stuck_bits, avg_pbp_elo_list, label='ELO', color='blue')
axs[2].plot(stuck_bits, avg_pbp_ga_list, label='GA', color='magenta')
axs[2].plot(stuck_bits, avg_pbp_bpso_list, label='BPSO', color='green')
#axs[2].set_title('PBP')
axs[2].set_xlabel('Proportion of elements with MSB stuck')
